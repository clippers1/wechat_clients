...
class Http(FastAPI):
    ...
    def send_text(
            ...
            receiver: str = Body("20954760419@chatroom", description="消息接收者，roomid 或者 wxid"),
    ...
    def revoke_msg_by_id(self, msgid) -> dict:
        """
            撤销
        """
        print("撤回消息id: ", msgid)
        ret = self.wcf.revoke_msg(int(msgid))
        return {"status": 0, "message": "成功", "data": {"revoke_msg": ret, "msgid": msgid}}
    
    
    def revoke_self_msg(self, roomid: str = Query("xxxxxxxx@chatroom", description="群的 id")) -> dict:
        res = self.query_sql("MSG1.db", "SELECT MsgSvrID, StrContent, CreateTime FROM MSG where StrTalker='"+ roomid +"' And IsSender=1 order by Sequence DESC limit 10")
        ret = {"status": -1, "message": "失败"}
        if res['status'] == 0:
            dataList: List[Dict] = res['data']['bs64']
            print(dataList)
            for item in dataList:
                if "分钟以内（Last" in item['StrContent']:
                    if 0 < time.time() - item['CreateTime'] < 120:
                        print('两分钟以内: ', item)
                        time.sleep(2)
